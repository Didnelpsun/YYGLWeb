<template>
  <div v-loading="WriteLoading">
    ...
    <el-dialog top="1%" :visible.sync="isShow" title="选择站点" width="80%" :before-close="DetailhandleClose">
      <ResourceList @selectResource="selectResource" :resourcetype="2"/>
    </el-dialog>
  </div>
</template>
...
import { AddResourceEquipment, UpdateResourceEquipment, GetEquipmentTypeList } from 'api/api'
  ...
  methods: {
    ...
    async add () {
      this.$refs.tableForm.validate(async (valid, msg) => {
        try {
          ...
          const res = await this.$axios.post(AddResourceEquipment, this.tableData)
    ...
    async edit () {
      this.$refs.tableForm.validate(async (valid, msg) => {
        try {
          this.WriteLoading = true
          const res = await this.$axios.put(UpdateResourceEquipment, this.tableData)
          this.WriteLoading = false
          if (res.error === true) {
            this.$message.warning('请补全信息')
            console.log(res.errorMessage)
