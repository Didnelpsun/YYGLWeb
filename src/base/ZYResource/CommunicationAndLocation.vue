        ...
        <div class="el-table__body-wrapper is-scrolling-none">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" width="100%">
            ...
            <tr class="el-table__row">
              ...
              <td><div class="cell">
                <div v-if="WriteState == 0 && isTask === 0">通讯与位置终端</div>
...
import {GetCommunicationAndLocationInfo, GetTaskEquipmentCommunicationAndLocationInfo, AddCommunicationAndLocation, EditCommunicationAndLocation,
  AddTaskEquipmentCommunicationAndLocation, EditTaskEquipmentCommunicationAndLocation} from 'api/SurveyManagement'
import {formatDate} from 'common/js/cache'
import {GlobalRes} from 'common/js/mixins'
import ResourceList from 'base/Resource/ResourceList'

export default {
  name: 'CommunicationAndLocationDetail',
  mixins: [GlobalRes],
  props: {
    DeviceID: {
      type: String,
  ...
  created () {
    ...
    if (this.WriteState) {
      this.Loading = true
      this.$axios.get(this.isTask ? GetTaskEquipmentCommunicationAndLocationInfo : GetCommunicationAndLocationInfo, {
        params: {
          id: this.DeviceID
        }
      }).then(res => {
  ...
  methods: {
    getDicList () {
      let arr = ['通讯与位置终端设备厂家', '通讯与位置终端设备型号']
      this.$axios.post(DictionaryInfoList, arr).then(res => {
        if (res.errorCode === '200') {
          this.$set(this.DicList, 'manufacturer', res.data.filter(i => { return i.type === '通讯与位置终端设备厂家' }))
          this.DicList.models = res.data.filter(i => { return i.type === '通讯与位置终端设备型号' })
        } else {
    ...
    SubAdd () {
      ...
      this.$refs.WriteForm.validate((vali, msg) => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.post(AddCommunicationAndLocation, this.WriteData).then(res => {
            this.Loading = false
    ...
    SubEdit () {
      ...
      this.$refs.WriteForm.validate(vali => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.put(EditCommunicationAndLocation, this.WriteData).then(res => {
            this.Loading = false
            if (res.errorCode !== '200') return this.$message.error(res.errorMessage)
            if (res.errorCode === '200') {
    ...
    SubTaskAdd (state) {
      ...
      this.$refs.WriteForm.validate((vali, msg) => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.post(AddTaskEquipmentCommunicationAndLocation, this.WriteData, {
            params: {
              censusstate: state
            }
          }).then(res => {
    ...
    SubTaskEdit (state) {
      ...
      this.$refs.WriteForm.validate(vali => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.put(EditTaskEquipmentCommunicationAndLocation, this.WriteData, {
            params: {
              censusstate: state
            }
          }).then(res => {
