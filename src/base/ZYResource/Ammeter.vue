        ...
        <div class="el-table__body-wrapper is-scrolling-none">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" width="100%">
            ...
            <tr class="el-table__row">
              ...
              <td><div class="cell">
                <div v-if="WriteState == 0 && isTask === 0">电表</div>
...
import {DictionaryInfoList, GetElectricMeterInfo} from 'api/api'
import {GetTaskElectricMeterInfo, AddElectricMeter, UpdateElectricMeter,
  AddTaskElectricMeter, UpdateTaskElectricMeter} from 'api/SurveyManagement'
...
export default {
  ...
  created () {
    ...
    if (this.WriteState) {
      this.Loading = true
      this.$axios.get(this.isTask ? GetTaskElectricMeterInfo : GetElectricMeterInfo, {
        params: {
          id: this.DeviceID
        }
      }).then(res => {
    ...
    SubAdd () {
      ...
      this.$refs.WriteForm.validate((vali, msg) => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.post(AddElectricMeter, this.WriteData).then(res => {
            this.Loading = false
    ...
    SubEdit () {
      ...
      this.$refs.WriteForm.validate(vali => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.put(UpdateElectricMeter, this.WriteData).then(res => {
            this.Loading = false
            if (res.errorCode !== '200') return this.$message.error(res.errorMessage)
            if (res.errorCode === '200') {
