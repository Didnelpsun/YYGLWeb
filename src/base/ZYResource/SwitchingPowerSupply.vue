...
export default {
  ...
  created () {
    ...
    if (this.WriteState) {
      this.Loading = true
      this.$axios.get(this.isTask ? GetSwitchingPowerSupplyTaskEquipmentInfo : GetSwitchingPowerSupplyInfo, {
        params: {
          id: this.DeviceID
        }
      }).then(res => {
    ...
    getDicList () {
      ...
      this.$axios.post(DictionaryInfoList, arr).then(res => {
        if (res.errorCode === '200') {
          let data = res.data
          this.$set(this.DicList, 'manufacturer', data.filter(i => { return i.type === '开关电源设备厂家' }))
    ...
    SubAdd (state) {
      ...
      this.$refs.WriteForm.validate((vali, msg) => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.post(AddSwitchingPowerSupplyTaskEquipment, this.WriteData, {
            params: {
              censusstate: state
            }
          }).then(res => {
    ...
    SubEdit (state) {
      ...
      this.$refs.WriteForm.validate(vali => {
        if (!vali) {
          ...
          this.Loading = true
          this.$axios.put(EditSwitchingPowerSupplyTaskEquipment, this.WriteData, {
            params: {
              censusstate: state
            }
          }).then(res => {
