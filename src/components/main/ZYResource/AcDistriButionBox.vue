<template>
  <div class="content">
    ...
    <div class="write" v-if="showWrite">
      <layuiTitle :title="WriteState === 0 ? '添加交流配电箱' : WriteState === 1 ? '修改交流配电箱' : '交流配电箱详情'"></layuiTitle>
...
import {GetAcDistriButionBoxList, GetAcDistriButionBoxExcel} from 'api/SurveyManagement'
import {exportMethod} from 'api/YDSZ'
import {formatDate} from 'common/js/cache'
import {GlobalRes} from 'common/js/mixins'
import layuiTitle from 'base/layui-title'
import ImgBox from 'base/ImgBox'
import Details from 'base/ZYResource/AcDistriButionBox'
...
export default {
  ...
  methods: {
    ...
    getDicList () {
      ...
      this.$axios.post(DictionaryInfoList, arr).then(res => {
        if (res.errorCode === '200') {
          this.DicList.powersupplymode = res.data.filter(i => { return i.type === '交流配电箱设备厂家' })
        } else {
    ...
    getTable1 () {
      this.Loading = true
      this.$axios.get(GetAcDistriButionBoxList, {
        params: {
          PageIndex: 1,
