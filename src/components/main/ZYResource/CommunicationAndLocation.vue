...
import {GetCommunicationAndLocationList, GetCommunicationAndLocationExcel} from 'api/SurveyManagement'
import {exportMethod} from 'api/YDSZ'
import {formatDate} from 'common/js/cache'
import {GlobalRes} from 'common/js/mixins'
import layuiTitle from 'base/layui-title'
import ImgBox from 'base/ImgBox'
import Details from 'base/ZYResource/CommunicationAndLocation'
  ...
  methods: {
    ...
    getDicList () {
      ...
      this.$axios.post(DictionaryInfoList, arr).then(res => {
        if (res.errorCode === '200') {
          this.DicList.powersupplymode = res.data.filter(i => { return i.type === '通讯与位置终端设备厂家' })
        } else {
    ...
    handleExport () {
      this.$confirm(`您确定要导出吗？`, '提示', {
        ...
        let myObj = {
          method: 'get',
          url: GetCommunicationAndLocationExcel,
          fileName: '通讯与位置终端',
          data: this.Query
        }
        exportMethod(myObj)
